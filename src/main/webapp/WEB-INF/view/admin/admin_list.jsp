...
<head>
	...
	$(function() {	
		...
		//搜索按钮
	  	$("#search-btn").click(function(){
	  		$('#dataList').datagrid('reload',{
	  			number:$("#search-colleges").combobox('getValue')
	  		});
	  	});
	...
	function f(){
		for(var i=0;i<colleges.length;i++){
 			if(colleges[i].name==$("#add_colleges").textbox('getText')){
 				document.getElementById("add_colleges").value=colleges[i].number;
 				//alert("123");
 			}
 		}
	}
	function g(){
		for(var i=0;i<colleges.length;i++){
 			if(colleges[i].number==$("#edit_colleges").textbox('getValue')){
 				document.getElementById("edit_colleges").value=colleges[i].name;
 				//alert("123");
 			}
 		}
	}
...
<body>
	...
	<div id="toolbar">
		...
		<div>
			...
			所属院校：
			<select id="search-colleges" class="easyui-combobox" style="width: 200px;">
   		...
   		<form id="addForm" method="post">
	    	<table id="addTable" cellpadding="8">
	    		...
	    		<tr >
	    			<td>院校名称:</td>
	    			<td>
	    				<select id="add_colleges"  class="easyui-combobox" style="width: 200px;" name="number" data-options="required:true, missingMessage:'请选择所属院校'">
    	...
    	<form id="editForm" method="post">
	    	...
	    	<table id="editTable" border=0  cellpadding="8" >
	    		...
	    		<tr >
	    			<td>院校名称:</td>
	    			<td>
	    				<select id="edit_colleges"  class="easyui-combobox" style="width: 200px;" name="number" data-options="required:true, missingMessage:'请选择所属院校'">
	    					<c:forEach items="${ colleges}" var="pro">
	    						<option value="${pro.number }">${pro.name}</option>
	    					</c:forEach>			
	    				</select>
