/*
 * Generated by the Jasper component of Apache Tomcat
 * Version: Apache Tomcat/7.0.47
 * Generated at: 2020-04-24 09:53:58 UTC
 * Note: The last modified time of this file was set to
 *       the last modified time of the source file after
 *       generation to assist with modification tracking.
 */
package org.apache.jsp.WEB_002dINF.view.news;

import javax.servlet.*;
import javax.servlet.http.*;
import javax.servlet.jsp.*;

public final class news_005flist_jsp extends org.apache.jasper.runtime.HttpJspBase
    implements org.apache.jasper.runtime.JspSourceDependent {

  private static final javax.servlet.jsp.JspFactory _jspxFactory =
          javax.servlet.jsp.JspFactory.getDefaultFactory();

  private static java.util.Map<java.lang.String,java.lang.Long> _jspx_dependants;

  private javax.el.ExpressionFactory _el_expressionfactory;
  private org.apache.tomcat.InstanceManager _jsp_instancemanager;

  public java.util.Map<java.lang.String,java.lang.Long> getDependants() {
    return _jspx_dependants;
  }

  public void _jspInit() {
    _el_expressionfactory = _jspxFactory.getJspApplicationContext(getServletConfig().getServletContext()).getExpressionFactory();
    _jsp_instancemanager = org.apache.jasper.runtime.InstanceManagerFactory.getInstanceManager(getServletConfig());
  }

  public void _jspDestroy() {
  }

  public void _jspService(final javax.servlet.http.HttpServletRequest request, final javax.servlet.http.HttpServletResponse response)
        throws java.io.IOException, javax.servlet.ServletException {

    final javax.servlet.jsp.PageContext pageContext;
    javax.servlet.http.HttpSession session = null;
    final javax.servlet.ServletContext application;
    final javax.servlet.ServletConfig config;
    javax.servlet.jsp.JspWriter out = null;
    final java.lang.Object page = this;
    javax.servlet.jsp.JspWriter _jspx_out = null;
    javax.servlet.jsp.PageContext _jspx_page_context = null;


    try {
      response.setContentType("text/html; charset=UTF-8");
      pageContext = _jspxFactory.getPageContext(this, request, response,
      			null, true, 8192, true);
      _jspx_page_context = pageContext;
      application = pageContext.getServletContext();
      config = pageContext.getServletConfig();
      session = pageContext.getSession();
      out = pageContext.getOut();
      _jspx_out = out;

      out.write("\r\n");
      out.write("\r\n");

	String path = request.getContextPath();
	String basePath = request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort()
			+ path + "/";

      out.write("\r\n");
      out.write("<!DOCTYPE html PUBLIC \"-//W3C//DTD HTML 4.01 Transitional//EN\" \"http://www.w3.org/TR/html4/loose.dtd\">\r\n");
      out.write("<html>\r\n");
      out.write("<head>\r\n");
      out.write("<meta charset=\"UTF-8\">\r\n");
      out.write("<title>用户列表</title>\r\n");
      out.write("<link rel=\"stylesheet\" type=\"text/css\"\r\n");
      out.write("\thref=\"");
      out.print(basePath);
      out.write("easyui/themes/default/easyui.css\">\r\n");
      out.write("<link rel=\"stylesheet\" type=\"text/css\"\r\n");
      out.write("\thref=\"");
      out.print(basePath);
      out.write("easyui/themes/icon.css\">\r\n");
      out.write("<link rel=\"stylesheet\" type=\"text/css\"\r\n");
      out.write("\thref=\"");
      out.print(basePath);
      out.write("easyui/css/demo.css\">\r\n");
      out.write("<script type=\"text/javascript\" src=\"");
      out.print(basePath);
      out.write("easyui/jquery.min.js\"></script>\r\n");
      out.write("<script type=\"text/javascript\"\r\n");
      out.write("\tsrc=\"");
      out.print(basePath);
      out.write("easyui/jquery.easyui.min.js\"></script>\r\n");
      out.write("<script type=\"text/javascript\"\r\n");
      out.write("\tsrc=\"");
      out.print(basePath);
      out.write("easyui/js/validateExtends.js\"></script>\r\n");
      out.write("<script type=\"text/javascript\">\r\n");
      out.write("$(function() {\t\r\n");
      out.write("\tvar table;\r\n");
      out.write("\t//datagrid初始化 \r\n");
      out.write("    $('#dataList').datagrid({ \r\n");
      out.write("        title:'新闻信息列表', \r\n");
      out.write("        iconCls:'icon-more',//图标 \r\n");
      out.write("        border: true, \r\n");
      out.write("        collapsible:false,//是否可折叠的 \r\n");
      out.write("        fit: true,//自动大小 \r\n");
      out.write("        method: \"post\",\r\n");
      out.write("        url:\"get_list?t=\"+new Date().getTime(),\r\n");
      out.write("        idField:'id', \r\n");
      out.write("        singleSelect:false,//是否单选 \r\n");
      out.write("        pagination:true,//分页控件 \r\n");
      out.write("        rownumbers:true,//行号 \r\n");
      out.write("        sortName:'id',\r\n");
      out.write("        sortOrder:'DESC', \r\n");
      out.write("        remoteSort: false,\r\n");
      out.write("        columns: [[  \r\n");
      out.write("\t\t\t{field:'chk',checkbox: true,width:50},\r\n");
      out.write("\t\t        {field:'id',title:'ID',width:50, sortable: true}, \r\n");
      out.write("\t\t        {field:'newsname',title:'新闻名称',width:100, sortable: true},\r\n");
      out.write("\t\t        {field:'newscontent',title:'新闻内容',width:150, sortable: true},\r\n");
      out.write("\t\t        {field:'newsfbz',title:'发布者',width:100},\r\n");
      out.write("\t\t        {field:'newstype',title:'新闻类型',width:100},\r\n");
      out.write("\t\t        {field:'newstime',title:'发布时间',width:100, sortable: true,align:\"center\"},\r\n");
      out.write("\t\t        \t\r\n");
      out.write(" \t\t]], \r\n");
      out.write("        toolbar: \"#toolbar\"\r\n");
      out.write("    }); \r\n");
      out.write("    //设置分页控件 \r\n");
      out.write("    var p = $('#dataList').datagrid('getPager'); \r\n");
      out.write("    $(p).pagination({ \r\n");
      out.write("        pageSize: 10,//每页显示的记录条数，默认为10 \r\n");
      out.write("        pageList: [10,20,30,50,100],//可以设置每页记录条数的列表 \r\n");
      out.write("        beforePageText: '第',//页数文本框前显示的汉字 \r\n");
      out.write("        afterPageText: '页    共 {pages} 页', \r\n");
      out.write("        displayMsg: '当前显示 {from} - {to} 条记录   共 {total} 条记录', \r\n");
      out.write("    }); \r\n");
      out.write("    //设置工具类按钮\r\n");
      out.write("    $(\"#add\").click(function(){\r\n");
      out.write("    \ttable = $(\"#addTable\");\r\n");
      out.write("    \t$(\"#addDialog\").dialog(\"open\");\r\n");
      out.write("    });\r\n");
      out.write("    //修改\r\n");
      out.write("    $(\"#edit\").click(function(){\r\n");
      out.write("    \ttable = $(\"#editTable\");\r\n");
      out.write("    \tvar selectRows = $(\"#dataList\").datagrid(\"getSelections\");\r\n");
      out.write("    \tif(selectRows.length != 1){\r\n");
      out.write("        \t$.messager.alert(\"消息提醒\", \"请选择一条数据进行操作!\", \"warning\");\r\n");
      out.write("        } else{\r\n");
      out.write("\t    \t$(\"#editDialog\").dialog(\"open\");\r\n");
      out.write("        }\r\n");
      out.write("    });\r\n");
      out.write("    //删除\r\n");
      out.write("    $(\"#delete\").click(function(){\r\n");
      out.write("    \tvar selectRows = $(\"#dataList\").datagrid(\"getSelections\");\r\n");
      out.write("    \tvar selectLength = selectRows.length;\r\n");
      out.write("    \tif(selectLength == 0){\r\n");
      out.write("        \t$.messager.alert(\"消息提醒\", \"请选择数据进行删除!\", \"warning\");\r\n");
      out.write("        } else{\r\n");
      out.write("        \tvar ids = [];\r\n");
      out.write("        \t$(selectRows).each(function(i, row){\r\n");
      out.write("        \t\tids[i] = row.id;\r\n");
      out.write("        \t});\t\r\n");
      out.write("        \t$.messager.confirm(\"消息提醒\", \"将删除与用户相关的所有数据，确认继续？\", function(r){\r\n");
      out.write("        \t\tif(r){\r\n");
      out.write("        \t\t\t$.ajax({\r\n");
      out.write("\t\t\t\t\t\ttype: \"post\",\r\n");
      out.write("\t\t\t\t\t\turl: \"delete\",\r\n");
      out.write("\t\t\t\t\t\tdata: {ids: ids},\r\n");
      out.write("\t\t\t\t\t\tdateType:'json',\r\n");
      out.write("\t\t\t\t\t\tsuccess: function(data){\r\n");
      out.write("\t\t\t\t\t\t\tif(data == \"success\"){\r\n");
      out.write("\t\t\t\t\t\t\t\t$.messager.alert(\"消息提醒\",\"删除成功!\",\"info\");\r\n");
      out.write("\t\t\t\t\t\t\t\t//刷新表格\r\n");
      out.write("\t\t\t\t\t\t\t\t$(\"#dataList\").datagrid(\"reload\");\r\n");
      out.write("\t\t\t\t\t\t\t\t $(\"#dataList\").datagrid(\"uncheckAll\"); \r\n");
      out.write("\t\t\t\t\t\t\t} else{\r\n");
      out.write("\t\t\t\t\t\t\t\t$.messager.alert(\"消息提醒\",data.msg,\"warning\");\r\n");
      out.write("\t\t\t\t\t\t\t\treturn;\r\n");
      out.write("\t\t\t\t\t\t\t}\r\n");
      out.write("\t\t\t\t\t\t}\r\n");
      out.write("\t\t\t\t\t});\r\n");
      out.write("        \t\t}\r\n");
      out.write("        \t});\r\n");
      out.write("        }\r\n");
      out.write("    });\r\n");
      out.write("    \r\n");
      out.write("  \t//设置添加窗口\r\n");
      out.write("    $(\"#addDialog\").dialog({\r\n");
      out.write("    \ttitle: \"添加用户\",\r\n");
      out.write("    \twidth: 400,\r\n");
      out.write("    \theight: 500,\r\n");
      out.write("    \ticonCls: \"icon-add\",\r\n");
      out.write("    \tmodal: true,\r\n");
      out.write("    \tcollapsible: false,\r\n");
      out.write("    \tminimizable: false,\r\n");
      out.write("    \tmaximizable: false,\r\n");
      out.write("    \tdraggable: true,\r\n");
      out.write("    \tclosed: true,\r\n");
      out.write("    \tbuttons: [\r\n");
      out.write("    \t\t{\r\n");
      out.write("\t\t\t\ttext:'添加',\r\n");
      out.write("\t\t\t\tplain: true,\r\n");
      out.write("\t\t\t\ticonCls:'icon-user_add',\r\n");
      out.write("\t\t\t\thandler:function(){\r\n");
      out.write("\t\t\t\t\tvar validate = $(\"#addForm\").form(\"validate\");\r\n");
      out.write("\t\t\t\t\tif(!validate){\r\n");
      out.write("\t\t\t\t\t\t$.messager.alert(\"消息提醒\",\"请检查你输入的数据!\",\"warning\");\r\n");
      out.write("\t\t\t\t\t\treturn;\r\n");
      out.write("\t\t\t\t\t} else{\r\n");
      out.write("\t\t\t\t\t\tvar data = $(\"#addForm\").serialize();\r\n");
      out.write("\t\t\t\t\t\tconsole.log(data);\r\n");
      out.write("\t\t\t\t\t\t$.ajax({\r\n");
      out.write("\t\t\t\t\t\t\ttype: \"post\",\r\n");
      out.write("\t\t\t\t\t\t\turl: \"add\",\r\n");
      out.write("\t\t\t\t\t\t\tdata: data,\r\n");
      out.write("\t\t\t\t\t\t\tdataType:'json',\r\n");
      out.write("\t\t\t\t\t\t\tsuccess: function(data){\r\n");
      out.write("\t\t\t\t\t\t\t\tif(data.type == \"success\"){\r\n");
      out.write("\t\t\t\t\t\t\t\t\t$.messager.alert(\"消息提醒\",\"添加成功!\",\"info\");\r\n");
      out.write("\t\t\t\t\t\t\t\t\t//关闭窗口\r\n");
      out.write("\t\t\t\t\t\t\t\t\t$(\"#addDialog\").dialog(\"close\");\r\n");
      out.write("\t\t\t\t\t\t\t\t\t//清空原表格数据\r\n");
      out.write("\t\t\t\t\t\t\t\t\t$(\"#add_name\").combobox('setValue', \"\");\r\n");
      out.write("\t\t\t\t\t\t\t\t\t$(\"#add_content\").textbox('setValue', \"\");\r\n");
      out.write("\t\t\t\t\t\t\t\t\t$(\"#add_address\").textbox('setValue', \"\");\r\n");
      out.write("\t\t\t\t\t\t\t\t\t$(\"#add_time\").datebox('setValue',\"\");\r\n");
      out.write("\t\t\t\t\t\t\t\t\t//重新刷新页面数据\r\n");
      out.write("\t\t\t\t\t\t  \t\t\t$('#dataList').datagrid(\"reload\");\r\n");
      out.write("\t\t\t\t\t\t\t\t\t\r\n");
      out.write("\t\t\t\t\t\t\t\t} else{\r\n");
      out.write("\t\t\t\t\t\t\t\t\t$.messager.alert(\"消息提醒\",data.msg,\"warning\");\r\n");
      out.write("\t\t\t\t\t\t\t\t\treturn;\r\n");
      out.write("\t\t\t\t\t\t\t\t}\r\n");
      out.write("\t\t\t\t\t\t\t}\r\n");
      out.write("\t\t\t\t\t\t});\r\n");
      out.write("\t\t\t\t\t}\r\n");
      out.write("\t\t\t\t}\r\n");
      out.write("\t\t\t},\r\n");
      out.write("\t\t\t\r\n");
      out.write("\t\t],\r\n");
      out.write("\t\tonClose: function(){\r\n");
      out.write("\t\t\t$(\"#add_content\").textbox('setValue', \"\");\r\n");
      out.write("\t\t\t$(\"#add_name\").combobox('setValue', \"\");\r\n");
      out.write("\t\t\t$(\"#add_address\").textbox('setValue', \"\");\r\n");
      out.write("\t\t}\r\n");
      out.write("    });\r\n");
      out.write("  \t\r\n");
      out.write("  \r\n");
      out.write("  \t\r\n");
      out.write("  \t//编辑用户信息\r\n");
      out.write("  \t$(\"#editDialog\").dialog({\r\n");
      out.write("  \t\ttitle: \"修改用户信息\",\r\n");
      out.write("    \twidth: 400,\r\n");
      out.write("    \theight: 500,\r\n");
      out.write("    \ticonCls: \"icon-edit\",\r\n");
      out.write("    \tmodal: true,\r\n");
      out.write("    \tcollapsible: false,\r\n");
      out.write("    \tminimizable: false,\r\n");
      out.write("    \tmaximizable: false,\r\n");
      out.write("    \tdraggable: true,\r\n");
      out.write("    \tclosed: true,\r\n");
      out.write("    \tbuttons: [\r\n");
      out.write("\t\t\t\r\n");
      out.write("    \t\t{\r\n");
      out.write("\t\t\t\ttext:'提交',\r\n");
      out.write("\t\t\t\tplain: true,\r\n");
      out.write("\t\t\t\ticonCls:'icon-edit',\r\n");
      out.write("\t\t\t\thandler:function(){\t\t\t\r\n");
      out.write("\t\t\t\t\tvar validate = $(\"#editForm\").form(\"validate\");\r\n");
      out.write("\t\t\t\t\tif(!validate){\r\n");
      out.write("\t\t\t\t\t\t$.messager.alert(\"消息提醒\",\"请检查你输入的数据!\",\"warning\");\r\n");
      out.write("\t\t\t\t\t\treturn;\r\n");
      out.write("\t\t\t\t\t} else{\r\n");
      out.write("\t\t\t\t\t\tvar data = $(\"#editForm\").serialize();\t\t\t\t\t\t\t\r\n");
      out.write("\t\t\t\t\t\t$.ajax({\r\n");
      out.write("\t\t\t\t\t\t\ttype: \"post\",\r\n");
      out.write("\t\t\t\t\t\t\turl: \"edit\",\r\n");
      out.write("\t\t\t\t\t\t\tdata: data,\r\n");
      out.write("\t\t\t\t\t\t\tdataType:'json',\r\n");
      out.write("\t\t\t\t\t\t\tsuccess: function(data){\r\n");
      out.write("\t\t\t\t\t\t\t\tif(data.type == \"success\"){\r\n");
      out.write("\t\t\t\t\t\t\t\t\t$.messager.alert(\"消息提醒\",\"修改成功!\",\"info\");\r\n");
      out.write("\t\t\t\t\t\t\t\t\t//关闭窗口\r\n");
      out.write("\t\t\t\t\t\t\t\t\t$(\"#editDialog\").dialog(\"close\");\r\n");
      out.write("\t\t\t\t\t\t\t\t\t//清空原表格数据\t\r\n");
      out.write("\t\t\t\t\t\t\t\t\t//重新刷新页面数据\r\n");
      out.write("\t\t\t\t\t\t  \t\t\t$('#dataList').datagrid(\"reload\");\r\n");
      out.write("\t\t\t\t\t\t  \t\t\t$('#dataList').datagrid(\"uncheckAll\");\r\n");
      out.write("\t\t\t\t\t\t\t\t\t\r\n");
      out.write("\t\t\t\t\t\t\t\t} else{\r\n");
      out.write("\t\t\t\t\t\t\t\t\t$.messager.alert(\"消息提醒\",data.msg,\"warning\");\r\n");
      out.write("\t\t\t\t\t\t\t\t\treturn;\r\n");
      out.write("\t\t\t\t\t\t\t\t}\r\n");
      out.write("\t\t\t\t\t\t\t}\r\n");
      out.write("\t\t\t\t\t\t});\r\n");
      out.write("\t\t\t\t\t}\r\n");
      out.write("\t\t\t\t}\r\n");
      out.write("\t\t\t},\r\n");
      out.write("\t\t\r\n");
      out.write("\t\t],\r\n");
      out.write("\t\tonBeforeOpen: function(){\r\n");
      out.write("\t\t\tvar selectRow = $(\"#dataList\").datagrid(\"getSelected\");\r\n");
      out.write("\t\t\t//设置值\r\n");
      out.write("\t\t\tvar time = selectRow.newstime.split(\"-\");\r\n");
      out.write("\t\t\tvar time1=time[2].split(\" \");\r\n");
      out.write("\t\t\tvar endtime = time[1] + \"/\" + time1[0] + \"/\" + time[0]+\" \"+time1[1];\r\n");
      out.write("\t\t\t$(\"#edit-id\").val( selectRow.id);\r\n");
      out.write("\t\t\t$(\"#edit_newsname\").textbox('setValue', selectRow.newsname);\r\n");
      out.write("\t\t\t$(\"#edit_newscontent\").textbox('setValue', selectRow.newscontent);\r\n");
      out.write("\t\t\t$(\"#edit_newstype\").textbox('setValue', selectRow.newstype);\r\n");
      out.write("\t\t\t$(\"#edit_newsfbz\").textbox('setValue', selectRow.newsfbz);\r\n");
      out.write("\t\t\t$(\"#edit_time\").datebox('setValue', endtime);\r\n");
      out.write("\t\t\t\t\t\t\r\n");
      out.write("\t\t\t\r\n");
      out.write("\t\t}\r\n");
      out.write("    });\r\n");
      out.write("\t\t//搜索按钮\r\n");
      out.write("\t\t$(\"#search-btn\").click(function() {\r\n");
      out.write("\t\t\t$('#dataList').datagrid('reload', {\r\n");
      out.write("\t\t\t\tnewstime : $(\"#search-time\").datebox('getValue'),\r\n");
      out.write("\t\t\t\tnewstype :$(\"#search-type\").textbox('getValue')\r\n");
      out.write("\t\t\t});\r\n");
      out.write("\t\t});\r\n");
      out.write("\r\n");
      out.write("\t});\r\n");
      out.write("</script>\r\n");
      out.write("</head>\r\n");
      out.write("<body>\r\n");
      out.write("\t<!-- 数据列表 -->\r\n");
      out.write("\t<table id=\"dataList\" cellspacing=\"0\" cellpadding=\"0\">\r\n");
      out.write("\r\n");
      out.write("\t</table>\r\n");
      out.write("\t<!-- 工具栏 -->\r\n");
      out.write("\t<div id=\"toolbar\">\r\n");
      out.write("\t\t<div style=\"float: left;\">\r\n");
      out.write("\t\t\t<a id=\"add\" href=\"javascript:;\" class=\"easyui-linkbutton\"\r\n");
      out.write("\t\t\t\tdata-options=\"iconCls:'icon-add',plain:true\">添加</a>\r\n");
      out.write("\t\t</div>\r\n");
      out.write("\t\t<div style=\"float: left;\" class=\"datagrid-btn-separator\"></div>\r\n");
      out.write("\t\t<div style=\"float: left;\">\r\n");
      out.write("\t\t\t<a id=\"edit\" href=\"javascript:;\" class=\"easyui-linkbutton\"\r\n");
      out.write("\t\t\t\tdata-options=\"iconCls:'icon-edit',plain:true\">修改</a>\r\n");
      out.write("\t\t</div>\r\n");
      out.write("\t\t<div style=\"float: left;\" class=\"datagrid-btn-separator\"></div>\r\n");
      out.write("\t\t<div>\r\n");
      out.write("\t\t\t<a id=\"delete\" href=\"javascript:;\" class=\"easyui-linkbutton\"\r\n");
      out.write("\t\t\t\tdata-options=\"iconCls:'icon-some-delete',plain:true\">删除</a> 时间查询： <input\r\n");
      out.write("\t\t\t\tid=\"search-time\" type=\"text\" class=\"easyui-datebox\" >\r\n");
      out.write("\t\t\t\t新闻类型： <input\r\n");
      out.write("\t\t\t\tid=\"search-type\" type=\"text\" class=\"easyui-textbox\" >\r\n");
      out.write("\t\t\t<a id=\"search-btn\" href=\"javascript:;\" class=\"easyui-linkbutton\"\r\n");
      out.write("\t\t\t\tdata-options=\"iconCls:'icon-search',plain:true\">搜索</a>\r\n");
      out.write("\t\t</div>\r\n");
      out.write("\t</div>\r\n");
      out.write("\r\n");
      out.write("\t<!-- 添加窗口 -->\r\n");
      out.write("\t<div id=\"addDialog\" style=\"padding: 10px;\">\r\n");
      out.write("\t\t<form id=\"addForm\" method=\"post\">\r\n");
      out.write("\t\t\t<table id=\"addTable\" cellpadding=\"8\">\r\n");
      out.write("\t\t\t<tr>\r\n");
      out.write("\t\t\t\t\t<td>新闻名称:</td>\r\n");
      out.write("\t\t\t\t\t<td><input id=\"add_newsname\"\r\n");
      out.write("\t\t\t\t\t\tstyle=\"width: 200px; height: 30px;\" class=\"easyui-textbox\"\r\n");
      out.write("\t\t\t\t\t\ttype=\"text\" name=\"newsname\"\r\n");
      out.write("\t\t\t\t\t\tdata-options=\"required:true\" /></td>\r\n");
      out.write("\t\t\t\t</tr>\r\n");
      out.write("\t\t\t\t\r\n");
      out.write("\t\t\t\t\r\n");
      out.write("\t\t\t\t<tr>\r\n");
      out.write("\t\t\t\t\t<td>新闻发布人:</td>\r\n");
      out.write("\t\t\t\t\t<td><input id=\"add_newsfbr\"\r\n");
      out.write("\t\t\t\t\t\tstyle=\"width: 200px; height: 30px;\" class=\"easyui-textbox\"\r\n");
      out.write("\t\t\t\t\t\ttype=\"text\" name=\"newsfbz\"\r\n");
      out.write("\t\t\t\t\t\tdata-options=\"required:true\" /></td>\r\n");
      out.write("\t\t\t\t</tr>\r\n");
      out.write("\t\t\t\t<tr>\r\n");
      out.write("\t\t\t\t\t<td>新闻类型:</td>\r\n");
      out.write("\t\t\t\t\t<td><input id=\"add_newstype\"\r\n");
      out.write("\t\t\t\t\t\tstyle=\"width: 200px; height: 30px;\" class=\"easyui-textbox\"\r\n");
      out.write("\t\t\t\t\t\ttype=\"text\" name=\"newstype\"\r\n");
      out.write("\t\t\t\t\t\tdata-options=\"required:true\" /></td>\r\n");
      out.write("\t\t\t\t</tr>\r\n");
      out.write("\t\t\t\t<tr>\r\n");
      out.write("\t\t\t\t\t<td>发布时间:</td>\r\n");
      out.write("\t\t\t\t\t<td><input id=\"add_time\" class=\"easyui-datebox\"\r\n");
      out.write("\t\t\t\t\t\tname=\"newstime\" data-options=\"required:true,showSeconds:false\"\r\n");
      out.write("\t\t\t\t\t\tvalue=\"\" style=\"width: 150px\"></input></td>\r\n");
      out.write("\t\t\t\t</tr>\r\n");
      out.write("\t\t\t\t<tr>\r\n");
      out.write("\t\t\t\t\t<td>新闻内容:</td>\r\n");
      out.write("\t\t\t\t\t<td><input class=\"easyui-textbox\" name=\"newscontent\" id=\"add_content\"\r\n");
      out.write("\t    data-options=\"multiline:true\" style=\"height:200px;width:200px\">\r\n");
      out.write("\t        </input></td>\r\n");
      out.write("\t\t\t</table>\r\n");
      out.write("\t\t</form>\r\n");
      out.write("\t</div>\r\n");
      out.write("\r\n");
      out.write("\r\n");
      out.write("\r\n");
      out.write("\t<!-- 修改窗口 -->\r\n");
      out.write("\t<div id=\"editDialog\" style=\"padding: 10px\">\r\n");
      out.write("\r\n");
      out.write("\t\t<form id=\"editForm\" method=\"post\">\r\n");
      out.write("\t\t\t<input type=\"hidden\" name=\"id\" id=\"edit-id\">\r\n");
      out.write("\t\t\t<table id=\"editTable\" border=0 cellpadding=\"8\">\r\n");
      out.write("\t\t\t\t<tr>\r\n");
      out.write("\t\t\t\t\t<td>新闻名称:</td>\r\n");
      out.write("\t\t\t\t\t<td><input id=\"edit_newsname\"\r\n");
      out.write("\t\t\t\t\t\tstyle=\"width: 200px; height: 30px;\" class=\"easyui-textbox\"\r\n");
      out.write("\t\t\t\t\t\ttype=\"text\" name=\"newsname\"\r\n");
      out.write("\t\t\t\t\t\tdata-options=\"required:true\" /></td>\r\n");
      out.write("\t\t\t\t</tr>\r\n");
      out.write("\t\t\t\t\r\n");
      out.write("\t\t\t\t</tr>\r\n");
      out.write("\t\t\t\t<tr>\r\n");
      out.write("\t\t\t\t\t<td>新闻发布人:</td>\r\n");
      out.write("\t\t\t\t\t<td><input id=\"edit_newsfbz\"\r\n");
      out.write("\t\t\t\t\t\tstyle=\"width: 200px; height: 30px;\" class=\"easyui-textbox\"\r\n");
      out.write("\t\t\t\t\t\ttype=\"text\" name=\"newsfbr\"\r\n");
      out.write("\t\t\t\t\t\tdata-options=\"required:true\" /></td>\r\n");
      out.write("\t\t\t\t</tr>\r\n");
      out.write("\t\t\t\t<tr>\r\n");
      out.write("\t\t\t\t\t<td>新闻类型:</td>\r\n");
      out.write("\t\t\t\t\t<td><input id=\"edit_newstype\"\r\n");
      out.write("\t\t\t\t\t\tstyle=\"width: 200px; height: 30px;\" class=\"easyui-textbox\"\r\n");
      out.write("\t\t\t\t\t\ttype=\"text\" name=\"newstype\"\r\n");
      out.write("\t\t\t\t\t\tdata-options=\"required:true\" /></td>\r\n");
      out.write("\t\t\t\t</tr>\r\n");
      out.write("\t\t\t\t<tr>\r\n");
      out.write("\t\t\t\t\t<td>发布时间:</td>\r\n");
      out.write("\t\t\t\t\t<td><input id=\"edit_time\" class=\"easyui-datebox\"\r\n");
      out.write("\t\t\t\t\t\tname=\"newstime\" data-options=\"required:true,showSeconds:false\"\r\n");
      out.write("\t\t\t\t\t\tvalue=\"\" style=\"width: 150px\"></input></td>\r\n");
      out.write("\t\t\t\t</tr>\r\n");
      out.write("\t\t\t\t<tr>\r\n");
      out.write("\t\t\t\t\t<td>新闻内容:</td>\r\n");
      out.write("\t\t\t\t\t<td><input class=\"easyui-textbox\" name=\"newscontent\" id=\"edit_content\"\r\n");
      out.write("\t    data-options=\"multiline:true\" style=\"height:200px;width:200px\">\r\n");
      out.write("\t        </input></td>\r\n");
      out.write("\t\t\t</table>\r\n");
      out.write("\t\t</form>\r\n");
      out.write("\t</div>\r\n");
      out.write("\r\n");
      out.write("</body>\r\n");
      out.write("</html>");
    } catch (java.lang.Throwable t) {
      if (!(t instanceof javax.servlet.jsp.SkipPageException)){
        out = _jspx_out;
        if (out != null && out.getBufferSize() != 0)
          try { out.clearBuffer(); } catch (java.io.IOException e) {}
        if (_jspx_page_context != null) _jspx_page_context.handlePageException(t);
        else throw new ServletException(t);
      }
    } finally {
      _jspxFactory.releasePageContext(_jspx_page_context);
    }
  }
}
